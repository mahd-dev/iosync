...
module.exports = {
	...
	client: {
		...
		branches: function (client_id) {
			...
			(function recursive_client_branches(tree, client_id, path = "/") {
				...
				for(let i in tree.branches)
					recursive_client_branches(tree.branches[i], client_id, (path + i + "/" ));
		...
		unbind: function (client_id, path) {
			...
			if(path){
				...
				// unbind the client
				delete current_branch.clients[client_id];
			...
			run: (url, params, session, client_callback) => {
				...
				let next_middleware = () => {
					...
					if (my_middlewares[i])
						...
						for (var c in query_listeners)
							if (query_listeners[c].url==url) query_listeners[c].callback(params, session, client_callback);
				};
				next_middleware();
