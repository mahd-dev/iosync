...
function make_branch() {
	return JSON.parse(JSON.stringify({
		...
		scope: "public", // access rights definition on this branch
	...
	branch: {
		...
		get_branch: (path, try_nearest = false) => {
			...
			for( let i = 0; i < exploded_path.length; i++ ){
				...
				if( !current_branch.branches[branch_name] ){
					// sub branch not exists

					if( current_branch.branches['?'] ){

						// seek to the dynamic sub branch
						current_branch = current_branch.branches['?']
					}else{
	...
	query: {
		...
		listener: {
			...
			add: (url, listener) => {
				...
				for (let q in query_listeners) {
					if (query_listeners.hasOwnProperty(q) && query_listeners[q].url==url) {
						query_listeners[q].callback = listener
						return q
					}
				}
			...
			run: (url, params, session, client_callback) => {
				...
				let url_exploded = explode(url)

				// get middlewares listening on this url
				let my_middlewares = []
				for(let m in middlewares){
					let m_url = explode(middlewares[m].url)
					for(let i in m_url){
						if( !url_exploded[i] ) break
						if(m_url[i] == '?') m_url[i] = url_exploded[i]
					}
					m_url = (url[0]=="/"?"/":"") + m_url.join('/')

					if( url.indexOf(m_url)===0 ) my_middlewares.push(middlewares[m])
				}
				...
				let next_middleware = () => {
					...
					if (my_middlewares[i])
						...
						for (let c in query_listeners){
							...
							let ql_url = explode(query_listeners[c].url)
							for(let u in ql_url){
								if( !url_exploded[u] ) break
								if(ql_url[u] == '?') ql_url[u] = url_exploded[u]
							}
							ql_url = (url[0]=="/"?"/":"") + ql_url.join('/')
							...
							if (ql_url==url){
								...
								url.indexOf(ql_url)==0 &&
								( !ql || ql.url.length < ql_url.length )
						...
						if(ql) ql.callback(url, params, session, client_callback)
