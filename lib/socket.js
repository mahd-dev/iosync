...
module.exports.listen = (io) => {
	...
	io.on("connection", (socket) => {
		...
		socket.on('patch', (patch, client_callback) => {
			...
			for(let p of patch){
				...
				for(let c in branch.clients){
					if( branch.clients[c].socket.id != socket.id && helpers.check_scope( branch.scope , branch.clients[c].socket.handshake.session.user, 1 ) ) {
		...
		socket.on('apply_params', (params, client_callback) => {
			...
			let branch = tree.branch.get_branch(params.path)
